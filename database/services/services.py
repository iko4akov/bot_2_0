...
async def create_user_db(conn):
    ...
    try:
        await conn.execute(
            CREATE_USER_COMMAND.format(username=POSTGRES_USER, password=POSTGRES_PASS)
        )
...
async def create_table_db(conn):
    """Создает базу данных."""
    try:
        await conn.execute(
            CREATE_DB_COMMAND.format(dbname=POSTGRES_DB, username=POSTGRES_USER)
        )
